...
var (
	...
	pathRe  = regexp.MustCompile(`(?:\./)?(?:\.ai|\.cursor|\.github|\.githooks|scripts|openclaw-dashboard|cmd|apps|api|design|docs|internal|deploy|migrations)(/[A-Za-z0-9_.*{},-]+)+|\.gitignore|(?:AGENTS|CLAUDE|CODEX|OPENCLAW|README|Team)\.md|go\.mod`)
...
func run(args []string, in io.Reader, out, errw io.Writer) int {
	...
	for i := 0; i < len(args); i++ {
		...
		if p, ok := set[a]; ok {
			if i+1 >= len(args) || strings.HasPrefix(args[i+1], "-") {
				fmt.Fprintf(errw, "missing value %s\n", a)
				return 2
			}
			i++
			*p = args[i]
	...
	if cmd == "hook" {
		return hook(in, out, root, evPath, baseDir, baseRef)
	}
	if cmd == "maps" {
		if err := mapsCmd(headDir, out); err != nil {
...
func build(root, baseDir, headDir, path, tool, claimed, evPath, baseRef string) (snap, error) {
	...
	if s.Base = baseRef; s.Base == "" {
		s.Base = git(headDir, "merge-base", "HEAD", "origin/main")
	}
	if s.Base == "" {
		s.Base = git(headDir, "rev-parse", "--verify", "origin/main^{commit}")
	}
	if s.Base == "" {
		s.Base = "unavailable"
	}
	...
	if baseDir != "" && filepath.Clean(baseDir) != filepath.Clean(headDir) {
		...
		bf, berr = loadFeatsRef(headDir, s.Base)
	}
	if len(berr) > 0 {
	...
	for _, h := range s.Relevant {
		s.Obligations = append(s.Obligations, h.Proofs...)
	}
...
func mapsCmd(headDir string, out io.Writer) error {
	...
	if len(feats) == 0 {
		return fmt.Errorf("sai-verify maps: no features under %s", mapDir)
	}
	s := snap{}
	fillMap(&s, feats, problems)
	if !s.MapValid {
		return fmt.Errorf("sai-verify maps: invalid map: %s", strings.Join(s.Problems, "; "))
	}
	cap := ""
	...
	for _, f := range feats {
		payload.Features = append(payload.Features, row{f.ID, f.File, f.Title, f.Desc, f.Subs, f.Ent, f.Proof, f.Gotchas})
	}
...
func loadFeatsRef(dir, ref string) ([]feat, []string) {
	if ref == "" || ref == "unavailable" || git(dir, "rev-parse", "--verify", ref+"^{commit}") == "" {
		return nil, []string{"trusted base unavailable"}
	}
	...
	for _, m := range linkRe.FindAllStringSubmatch(idx, -1) {
		if m[1] != "README.md" {
			os.WriteFile(filepath.Join(d, m[1]), []byte(git(dir, "show", ref+":"+mapDir+"/"+m[1])), 0644)
		}
	}
...
func mapHash(dir string) string {
	...
	for _, f := range fs {
		for _, p := range f.Paths {
			...
			alts := []string{p}
			if i, j := strings.IndexByte(p, '{'), strings.LastIndexByte(p, '}'); i > 0 && j > i {
				alts = nil
				for _, a := range strings.Split(p[i+1:j], ",") {
					alts = append(alts, p[:i]+a+p[j+1:])
				}
			}
			for _, e := range alts {
				if k := strings.IndexAny(e, "*?"); k > 1 {
					e = filepath.Clean(e[:k])
				}
	...
	for _, p := range uniq(append(roots, mapDir, "cmd", ".cursor/hooks", ".cursor/hooks.json", ".github/policy", ".github/workflows", "go.mod")) {
		filepath.Walk(filepath.Join(dir, p), func(fp string, inf os.FileInfo, err error) error {
			if err == nil && inf != nil && !inf.IsDir() {
				...
				if !seen[rel] && !strings.HasPrefix(rel, ".ai/runs/") && !strings.Contains(rel, "__pycache__") && !strings.Contains(rel, "node_modules") && !strings.HasSuffix(rel, ".pyc") {
					seen[rel] = true
					b, _ := os.ReadFile(fp)
					h.Write([]byte(rel))
					h.Write(b)
	...
	for _, f := range strings.Split(git(dir, "ls-files", "-z", "--others", "--exclude-standard"), "\x00") {
		if f != "" && !seen[f] && !strings.HasPrefix(f, ".ai/runs/") && !strings.Contains(f, "__pycache__") && !strings.Contains(f, "node_modules") && !strings.HasSuffix(f, ".pyc") {
			seen[f] = true
...
func bindEvidence(s *snap, evPath, headDir string) {
	...
	if evPath == "" {
		evPath = filepath.Join(git(headDir, "rev-parse", "--absolute-git-dir"), "sai-verify-evidence.json")
	}
	...
	if json.Unmarshal(b, &ev) != nil {
		s.MaintReason = "malformed evidence"
		return
	}
	...
	for _, d := range ds {
		...
		if str(m["result"]) != "SKIP" && (m["stdout"] == nil || m["stderr"] == nil) {
			ioOK = false
		}
	}
	switch {
	...
	case ebase == "" || s.Base == "" || ebase != s.Base:
		s.MaintReason = "evidence base mismatch"
	case head != s.Head:
	...
	case sweep != "clean" || len(um) > 0 || len(s.Unmapped) > 0:
		s.MaintStatus, s.MaintReason = "incomplete", "source sweep not clean"
	case n != 0:
		s.MaintStatus, s.MaintReason = "bound", "live-drive failures"
	default:
		s.MaintStatus, s.MaintRequired, s.MaintReason, s.Completeness, s.EvidenceBound = "bound", false, "", "proven", true
	}
}
func unmapped(root string, fs []feat) []string {
	...
	for _, ft := range fs {
		for _, p := range ft.Paths {
			...
			for _, e := range alts {
				if k := strings.IndexAny(e, "*?"); k >= 0 {
					prefs = append(prefs, e)
					if strings.HasSuffix(e, "/*") {
						prefs = append(prefs, filepath.Clean(e[:k]))
					}
				} else {
					exact[e] = true
				}
	...
	for _, f := range strings.Split(blob, "\x00") {
		if f == "" || strings.HasPrefix(f, ".ai/runs/") || strings.Contains(f, "__pycache__") || strings.Contains(f, "node_modules") || strings.HasSuffix(f, ".pyc") {
			continue
		}
		hit := exact[f]
		if !hit {
			for _, p := range prefs {
				...
				} else if p != "." && (f == p || strings.HasPrefix(f, p+"/")) {
					hit = true
					break
				}
			}
...
func preserve(base, head []feat) (missing, weakened []string) {
	...
	for _, f := range base {
		...
		hf := hm[f.ID]
		if has(f.ID) && (lost(func() []string {
			var o []string
			for _, id := range subIDs(f) {
				if !rm[id] {
					o = append(o, id)
				}
			}
			return o
		}(), subIDs(hf)) || lost(f.Ent, hf.Ent) || lost(f.Proof, hf.Proof)) {
			weakened = append(weakened, f.ID)
		}
		for _, s := range f.Subs {
			if !rm[s[0]] && !has(s[0]) {
...
func checkHooks(root string) (ok, pre, post, stop bool, matcher string, failClosed bool, events []string) {
	...
	good := func(name string) bool {
		for _, h := range doc.Hooks[name] {
			if !strings.Contains(h.Command, "sai-verify") || matchEv[name] && h.Matcher != ".*" && h.Matcher != "^.*$" {
				continue
			}
			return h.FailClosed
		}
...
func hookCtx(s snap) string {
	...
	if b.Len() > capn {
		return b.String()[:capn]
	}
	return b.String()
}
func st(ok bool) string {
	if ok {
		return "PASS"
	}
	return "FAIL"
...
func evOK(evPath, headDir string) bool {
	ap, err := filepath.Abs(evPath)
	if err != nil || ap == "" || !strings.HasSuffix(strings.ToLower(ap), ".json") {
		return false
	}
	if st, e := os.Lstat(ap); e == nil && (!st.Mode().IsRegular() || st.Mode()&os.ModeSymlink != 0) {
		return false
	}
	par, err := filepath.EvalSymlinks(filepath.Dir(ap))
	if err != nil {
		return false
	}
	under := func(r string) bool {
		if r == "" {
			return false
		}
		r, _ = filepath.Abs(r)
		...
		return par == r || strings.HasPrefix(par, r+string(filepath.Separator))
	}
	gd, tmp, rt := git(headDir, "rev-parse", "--absolute-git-dir"), os.TempDir(), os.Getenv("RUNNER_TEMP")
	return (!under(headDir) || under(gd)) && (under(gd) || under(tmp) || under(rt) || par == "/tmp" || strings.HasPrefix(par, "/tmp"+string(filepath.Separator)))
}
func writeProof(w io.Writer, s snap, evPath, headDir string) {
	...
	rp := evPath
	if rp == "" {
		rp = filepath.Join(git(headDir, "rev-parse", "--absolute-git-dir"), "sai-verify-evidence.json")
	}
	if b, err := os.ReadFile(rp); err == nil {
		var ev map[string]interface{}
		_ = json.Unmarshal(b, &ev)
		pass, fail = ev["pass"], ev["fail"]
		if ds, _ := ev["drives"].([]interface{}); ds != nil {
			drives = ds
			for _, d := range ds {
				m, _ := d.(map[string]interface{})
				n, r := str(m["name"]), str(m["result"])
				if strings.Contains(n, "::gotest") {
					gt = st(r == "PASS") + " go-test"
				}
				if strings.Contains(n, "::govet") {
					gv = st(r == "PASS") + " go-vet"
				}
			}
		}
	}
	eb := "REQUIRED evidence-bound"
	if s.Completeness == "proven" && s.EvidenceBound {
		eb = "PASS evidence-bound " + s.MaintHead + " " + s.MaintMapHash
	} else {
		gt, gv = "UNEVALUATED go-test", "UNEVALUATED go-vet"
	}
	wm := "UNPROVEN whole-repo-maintenance"
	if s.Completeness == "proven" {
		wm = "PASS whole-repo-maintenance"
	}
	jp, art, bs := "", "FAIL proof-artifacts", "bash"
	if evPath != "" {
		jp = filepath.Join(filepath.Dir(evPath), "sai-verify-proof.json")
		if evOK(jp, headDir) {
			art = "PASS proof-artifacts"
		}
	}
	stt, n := git(headDir, "diff", "--shortstat", "origin/main...HEAD"), -1
	if i := strings.Index(stt, "insertion"); i > 0 {
		fs := strings.Fields(stt[:i])
		if len(fs) > 0 {
			fmt.Sscanf(fs[len(fs)-1], "%d", &n)
		}
	}
	lb := "UNEVALUATED line-budget"
	if stt != "" && n >= 0 {
		if n <= 1200 {
			lb = fmt.Sprintf("PASS line-budget %d", n)
		} else {
			lb = fmt.Sprintf("FAIL line-budget %d", n)
		}
	}
	g := map[string]interface{}{"repo": s.Repo, "base": s.Base, "head": s.Head, "map_files": s.MapFiles, "map_features": s.MapFeatures, "map_subfeatures": s.MapSubs, "map_entry_points": s.MapEntries, "impl_files": s.ImplFiles, "impl_loc": s.ImplLOC, "impl_funcs": s.ImplFuncs, "map_parse": st(s.MapValid) + " map-parse", "hooks": st(s.HooksOK) + " hooks", "preserve": st(s.PreserveOK) + " preserve", "hook_pre": st(s.HookPre) + " hook-pre", "hook_post": st(s.HookPost) + " hook-post", "whole_repo_maintenance": wm, "maintenance_status": s.MaintStatus, "maintenance_head": s.MaintHead, "maintenance_map_hash": s.MaintMapHash, "whole_repo_completeness": s.Completeness, "evidence_bound": eb, "fail_closed_evidence": eb, "completeness_derived": st(len(s.Unmapped) == 0) + " completeness-derived", "hash_derived": st(bytes.Contains(src, []byte("for _, p := range f.Paths")) && bytes.Contains(src, []byte(`".github/policy"`)) && len(mapHash(headDir)) == 32) + " hash-derived", "structured_recipes": st(!bytes.Contains(src, []byte(`"`+bs+`", "-lc"`)) && !bytes.Contains(src, []byte(`Command("`+bs+`"`))) + " structured-recipes", "trusted_base_policy": st(bytes.Contains(anti, []byte("pull_request_target")) && bytes.Contains(anti, []byte("trusted/.github/policy/anti-regression.py"))) + " trusted-base-policy", "hook_integrity": st(s.HooksOK && s.HookFailClosed) + " hook-integrity", "pinned_actions": st(regexp.MustCompile(`actions/(?:checkout|setup-go|upload-artifact)@[0-9a-f]{40}`).Match(audit)) + " pinned-actions", "adversarial_tests": gt, "go_test": gt, "go_vet": gv, "line_budget": lb, "whole_repo_map": st(len(s.Unmapped) == 0) + " whole-repo-map", "proof_artifacts": art, "density": "one parser; derived completeness/hash; no second manifest", "unreachable": s.Unreachable, "unmapped": s.Unmapped, "live_drive_pass": pass, "live_drive_fail": fail}
	...
	for _, u := range s.Unreachable {
		fmt.Fprintln(w, "UNREACHABLE", u)
	}
	for _, u := range s.Unmapped {
		fmt.Fprintln(w, "UNPROVEN unmapped", u)
	}
	if s.Err != "" {
		fmt.Fprintln(w, "FAIL", s.Err)
	}
	if art == "PASS proof-artifacts" {
		b, _ := json.MarshalIndent(g, "", "  ")
		tf := jp + ".tmp"
		os.Remove(tf)
		f, err := os.OpenFile(tf, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0644)
		if err == nil {
			_, err = f.Write(b)
			f.Close()
			if err != nil || os.Rename(tf, jp) != nil {
				os.Remove(tf)
			}
		}
	}
...
func relTool(f feat, tool string) bool {
	...
	if strings.Contains(t, "mcp") {
		return strings.Contains(blob, "mcp") || f.ID == "verify-sai"
	}
	if strings.Contains(t, "shell") || t == "bash" {
		return strings.Contains(blob, "shell") || strings.Contains(blob, "scripts/") || f.ID == "verify-sai"
	}
	return true
}
...
func okTok(a string) bool {
	if a == "" || a == ".." || strings.Contains(a, "/../") || strings.HasPrefix(a, "../") {
		return false
	}
	for _, c := range a {
...
func relPath(p string) bool {
	return p != "" && !filepath.IsAbs(p) && !strings.Contains(p, "..") && !strings.HasPrefix(p, "/") && okTok(p)
}
func allowBin(p string) bool {
	if !relPath(p) {
		return false
	}
	for _, pre := range []string{"scripts/verify-", "openclaw-dashboard/scripts/verify-", "openclaw-dashboard/tests/smoke/"} {
		if r := strings.TrimPrefix(p, pre); r != p && !strings.Contains(r, "/") {
			return true
		}
	}
	switch p {
	case "scripts/agent-report", "scripts/agent-sync-drive", "scripts/agent-contract-pr-review", "scripts/agent-verify-caps":
		return true
	}
	return false
}
func parseRecipe(p string) (recipe, error) {
	i := strings.Index(p, "::")
	if i < 0 {
		return recipe{}, fmt.Errorf("unstructured")
	}
	pre, j := strings.TrimSpace(p[:i]), strings.LastIndex(strings.TrimSpace(p[:i]), "**")
	if pre != "" && !(strings.HasPrefix(pre, "**") && strings.Count(pre, "**") >= 2 && j >= 0 && (strings.TrimSpace(pre[j+2:]) == "" || strings.TrimSpace(pre[j+2:]) == ".")) {
		return recipe{}, fmt.Errorf("unstructured")
	}
	fs := strings.Fields(p[i+2:])
	if len(fs) == 0 {
		return recipe{}, fmt.Errorf("empty")
	}
	r := recipe{op: fs[0], to: 60 * time.Second}
	for _, t := range fs[1:] {
		if strings.HasPrefix(t, "expect=") {
			fmt.Sscanf(t[7:], "%d", &r.want)
			continue
		}
		if strings.HasPrefix(t, "timeout=") {
			...
			fmt.Sscanf(t[8:], "%d", &n)
			if n <= 0 {
				r.to = time.Millisecond
			} else {
				r.to = time.Duration(n) * time.Second
			}
			continue
		}
...
func (r recipe) err() error {
	...
	case "exec":
		if n == 0 || !allowBin(a0) || a0 == "scripts/agent-report" && (r.read == "" || !relPath(r.read)) {
			return fmt.Errorf("bin")
		}
		for _, a := range r.argv[1:] {
			if !okTok(a) {
				return fmt.Errorf("arg")
			}
		}
	case "py":
		if n == 0 || !relPath(a0) || !strings.HasSuffix(a0, ".py") || (!strings.HasPrefix(a0, ".github/") && !strings.HasPrefix(a0, "scripts/")) {
			return fmt.Errorf("py")
		}
		for _, a := range r.argv {
			if a == "-c" || !okTok(a) {
				return fmt.Errorf("py")
	...
	case "gotest":
		for _, a := range r.argv {
			if a != "-race" && a != "./..." && !strings.HasPrefix(a, "./cmd/") && a != "-count=1" && !strings.HasPrefix(a, "-timeout=") {
				return fmt.Errorf("go")
			}
		}
	case "govet":
		for _, a := range r.argv {
			if a != "./..." && !strings.HasPrefix(a, "./cmd/") {
				return fmt.Errorf("vet")
			}
		}
	case "sai":
		if n == 0 {
			return fmt.Errorf("sai")
		}
		switch a0 {
		case "snapshot", "proof", "doctor", "relevant", "preserve":
		default:
			return fmt.Errorf("sai")
		}
		for _, a := range r.argv[1:] {
...
func recipeEnv(tmp string) []string {
	...
	for _, k := range []string{"GOROOT", "GOPATH", "GOMODCACHE"} {
		if v := os.Getenv(k); v != "" {
			env = append(env, k+"="+v)
		}
	}
...
func runRecipe(r recipe, dir string) (code int, stdout, stderr, note string) {
	...
	switch r.op {
	case "exists":
		for _, p := range r.argv {
			if _, err := os.Stat(filepath.Join(dir, p)); err != nil {
				return 1, "", err.Error(), "missing"
			}
		}
		return 0, "", "", ""
	case "contains":
		b, err := os.ReadFile(filepath.Join(dir, r.argv[0]))
		if err != nil {
			return 1, "", err.Error(), "read"
		}
		if !strings.Contains(string(b), strings.Join(r.argv[1:], " ")) {
			return 1, "", "", "needle"
		}
		return 0, "", "", ""
	case "sai":
		var o, e bytes.Buffer
		return run(append(append([]string{}, r.argv...), "--root", dir), bytes.NewReader(nil), &o, &e), o.String(), e.String(), ""
	}
	ctx, cancel := context.WithTimeout(context.Background(), r.to)
	defer cancel()
	tmp, _ := os.MkdirTemp("", "sai-r-")
	defer os.RemoveAll(tmp)
	argv := r.argv
	switch r.op {
	case "json":
		argv = []string{"python3", "-m", "json.tool", r.argv[0]}
	case "py":
		argv = append([]string{"python3"}, r.argv...)
	case "gotest":
		argv = append([]string{"go", "test"}, r.argv...)
	case "govet":
		argv = append([]string{"go", "vet"}, r.argv...)
	case "exec":
	default:
		return 1, "", "", "op"
	}
	c := exec.CommandContext(ctx, argv[0], argv[1:]...)
	c.Dir, c.Env = dir, recipeEnv(tmp)
	var o, e bytes.Buffer
	var before []os.DirEntry
	c.Stdout, c.Stderr = &o, &e
	if r.read != "" {
		before, _ = os.ReadDir(filepath.Join(dir, r.read))
	}
	err := c.Run()
	if ctx.Err() == context.DeadlineExceeded {
		return 1, o.String(), e.String(), "timeout"
	}
	if err != nil {
		...
		after, _ := os.ReadDir(filepath.Join(dir, r.read))
		seen := map[string]bool{}
		...
		for _, de := range after {
			...
			b, _ := os.ReadFile(filepath.Join(dir, r.read, de.Name()))
			if r.has == "" || bytes.Contains(b, []byte(r.has)) {
				hit = true
				break
			}
		}
...
func driveCmd(s snap, headDir, evPath string, out io.Writer) int {
	...
	rows, pass, fail := []map[string]string{}, 0, 0
	for _, f := range fs {
		for _, p := range f.Proof {
			rec, err := parseRecipe(p)
			so, se, note, code, res := "", "", "", 1, "FAIL"
			if err != nil {
				note, fail = err.Error(), fail+1
			} else if len(rec.argv) > 0 && rec.argv[0] == "scripts/agent-report" && (rec.has == "" || !okTok(rec.has)) {
				note, fail = "has", fail+1
			} else {
				code, so, se, note = runRecipe(rec, headDir)
				if code == rec.want {
	...
	if evPath == "" {
		if gd := git(headDir, "rev-parse", "--absolute-git-dir"); gd != "" {
			evPath = filepath.Join(gd, "sai-verify-evidence.json")
		} else {
			evPath = "sai-verify-evidence.json"
		}
	}
	...
	if !evOK(evPath, headDir) {
		fmt.Fprintln(out, `{"error":"evidence path"}`)
		return 1
	}
	tf := evPath + ".tmp"
	os.Remove(tf)
	...
	_, err = f.Write(b)
	f.Close()
	if err != nil || os.Rename(tf, evPath) != nil {
		os.Remove(tf)
		fmt.Fprintln(out, `{"error":"evidence path"}`)
		return 1
	}

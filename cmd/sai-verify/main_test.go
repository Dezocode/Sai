...
func TestLinkedWorktreeHook(t *testing.T) {
	...
	hook(); a, _ := filepath.Glob(glob); for _, pth := range a { if bs == "" || !strings.HasPrefix(filepath.Base(pth), "sai-verify-"+bs+"-") { t.Fatal("bootstrap compiled candidate", a) } }
	...
	for _, pth := range b2 { if bs == "" || !strings.HasPrefix(filepath.Base(pth), "sai-verify-"+bs+"-") { t.Fatal("dirty candidate compiled", a, b2) } }; if s2, _ := build(wt, "", wt, "", "", "", evp, ""); s2.EvidenceBound { t.Fatal("dirty kernel still bound") }

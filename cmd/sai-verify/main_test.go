...
func TestFuturePRAndHooks(t *testing.T) {
	...
	proof := bytes.Buffer{}
	run([]string{"proof", "--root", r, "--evidence", filepath.Join(t.TempDir(), "no-evidence.json")}, bytes.NewReader(nil), &proof, bytes.NewBuffer(nil))
	ps := proof.String()
	...
	if c, o := drive(t.TempDir(), "::exists .cursor/hooks.json"); c != 0 || !strings.Contains(o, `"result": "PASS"`) { t.Fatal("drive", o) }; dabs := t.TempDir(); mini(t, dabs, map[string]string{"alpha.md": featBody("alpha", "", "::exists .cursor/hooks.json", "e")}); if run([]string{"drive", "--root", dabs, "--head", dabs, "--evidence", "/etc/passwd"}, bytes.NewReader(nil), bytes.NewBuffer(nil), bytes.NewBuffer(nil)) == 0 { t.Fatal("abs evidence") }; os.WriteFile(filepath.Join(dabs, "go.mod"), []byte("module x\n"), 0644); if run([]string{"drive", "--root", dabs, "--head", dabs, "--evidence", filepath.Join(dabs, "go.mod")}, bytes.NewReader(nil), bytes.NewBuffer(nil), bytes.NewBuffer(nil)) == 0 { t.Fatal("repo evidence") }; os.WriteFile(filepath.Join(dabs, "trap.json"), []byte("{}\n"), 0644); if run([]string{"drive", "--root", dabs, "--head", dabs, "--evidence", filepath.Join(dabs, "trap.json")}, bytes.NewReader(nil), bytes.NewBuffer(nil), bytes.NewBuffer(nil)) == 0 { t.Fatal("repo json") }; lk := filepath.Join(t.TempDir(), "e.json"); os.Symlink("/etc/passwd", lk); if run([]string{"drive", "--root", dabs, "--head", dabs, "--evidence", lk}, bytes.NewReader(nil), bytes.NewBuffer(nil), bytes.NewBuffer(nil)) == 0 { t.Fatal("symlink evidence") }; if run([]string{"doctor", "--evidence"}, bytes.NewReader(nil), bytes.NewBuffer(nil), bytes.NewBuffer(nil)) == 0 { t.Fatal("bare evidence") }; rt, e2 := os.MkdirTemp("/var/tmp", "sai-rt-"); if e2 != nil { if h := os.Getenv("HOME"); h != "" { rt, e2 = os.MkdirTemp(h, "sai-rt-") } }; if e2 == nil { os.Setenv("RUNNER_TEMP", rt); if run([]string{"drive", "--root", dabs, "--head", dabs, "--evidence", "/etc/passwd"}, bytes.NewReader(nil), bytes.NewBuffer(nil), bytes.NewBuffer(nil)) == 0 { t.Fatal("abs+rt") }; if run([]string{"drive", "--root", dabs, "--head", dabs, "--evidence", filepath.Join(rt, "e.json")}, bytes.NewReader(nil), bytes.NewBuffer(nil), bytes.NewBuffer(nil)) != 0 { t.Fatal("runner temp") }; os.Unsetenv("RUNNER_TEMP"); os.RemoveAll(rt) }
...
func TestLinkedWorktreeHook(t *testing.T) {
	...
	for _, rel := range []string{".cursor/hooks/sai-verify.sh", "cmd/sai-verify/main.go", "go.mod", ".cursor/skills/verify-sai/features/verify-sai.md"} { b, _ := os.ReadFile(filepath.Join(r, rel)); os.MkdirAll(filepath.Join(wt, filepath.Dir(rel)), 0755); os.WriteFile(filepath.Join(wt, rel), b, 0755) }
	head, _ := exec.Command("git", "-C", wt, "rev-parse", "HEAD").Output(); gd, _ := exec.Command("git", "-C", wt, "rev-parse", "--absolute-git-dir").Output(); mb, _ := exec.Command("git", "-C", wt, "merge-base", "HEAD", "origin/main").Output(); gds, hs, bs := strings.TrimSpace(string(gd)), strings.TrimSpace(string(head)), strings.TrimSpace(string(mb)); glob := filepath.Join(gds, "sai-verify-"+hs+"-*"); ms, _ := filepath.Glob(glob); for _, m := range ms { os.Remove(m) }
	mg, _ := exec.Command("git", "-C", wt, "show", bs+":cmd/sai-verify/main.go").Output(); gm, _ := exec.Command("git", "-C", wt, "show", bs+":go.mod").Output(); sid := fmt.Sprintf("%x", sha256.Sum256(append(mg, gm...)))[:16]; allow := "sai-verify-"+bs+"-"+sid
	...
	hook(); a, _ := filepath.Glob(glob); for _, pth := range a { if bs == "" || filepath.Base(pth) != allow { t.Fatal("bootstrap compiled candidate", a, allow) } }; if st, err := os.Stat(filepath.Join(gds, allow)); err != nil || st.Size() < 4096 { t.Fatal("missing BASE cache", allow) }
	decoy := filepath.Join(gds, "sai-verify-"+hs+"-cafec0decafe0000"); os.WriteFile(decoy, []byte("x"), 0755); os.Remove(filepath.Join(gds, allow)); hook(); if st, err := os.Stat(decoy); err != nil || st.Size() != 1 { t.Fatal("decoy consumed", decoy) }; if st, err := os.Stat(filepath.Join(gds, allow)); err != nil || st.Size() < 4096 { t.Fatal("BASE cache missing after decoy", allow) }; d, _ := filepath.Glob(glob); for _, pth := range d { b := filepath.Base(pth); if b != allow && b != filepath.Base(decoy) { t.Fatal("unexpected cache", d, allow) } }
	s0, _ := build(wt, "", wt, "", "", "", "", ""); evp := filepath.Join(t.TempDir(), "bound.json"); os.WriteFile(evp, []byte(`{"repo":"`+s0.Repo+`","base":"`+s0.Base+`","head":"`+s0.Head+`","map_hash":"`+mapHash(wt)+`","sweep":"clean","fail":0,"pass":1,"unmapped":[],"drives":[{"result":"PASS","stdout":"","stderr":""}]}`), 0644); if s1, _ := build(wt, "", wt, "", "", "", evp, ""); !s1.EvidenceBound { t.Fatal("clean evidence", s1.MaintReason) }; sp := filepath.Join(wt, "scripts/verify-semantic-hierarchy"); sb, _ := os.ReadFile(sp); os.WriteFile(sp, append(sb, []byte("\n#DIRTY\n")...), 0644); if s3, _ := build(wt, "", wt, "", "", "", evp, ""); s3.EvidenceBound { t.Fatal("dirty script still bound") }; os.WriteFile(sp, sb, 0644); rg := filepath.Join(wt, ".ai/agents/registry.json"); rb, _ := os.ReadFile(rg); os.WriteFile(rg, append(rb, []byte("\n")...), 0644); if s4, _ := build(wt, "", wt, "", "", "", evp, ""); s4.EvidenceBound { t.Fatal("dirty registry still bound") }; os.WriteFile(rg, rb, 0644); stg := filepath.Join(wt, ".ai/stages/01_intake/CONTEXT.md"); stb, _ := os.ReadFile(stg); os.WriteFile(stg, append(stb, []byte("\n")...), 0644); if s6, _ := build(wt, "", wt, "", "", "", evp, ""); s6.EvidenceBound { t.Fatal("dirty stage still bound") }; os.WriteFile(stg, stb, 0644); nob := filepath.Join(t.TempDir(), "norepo.json"); os.WriteFile(nob, []byte(`{"head":"`+s0.Head+`","map_hash":"`+mapHash(wt)+`","sweep":"clean","fail":0,"pass":1,"unmapped":[],"drives":[{"result":"PASS","stdout":"","stderr":""}]}`), 0644); if s5, _ := build(wt, "", wt, "", "", "", nob, ""); s5.EvidenceBound { t.Fatal("empty repo bound") }; hU := mapHash(wt); os.WriteFile(filepath.Join(wt, "unexpected.txt"), []byte("x"), 0644); if mapHash(wt) == hU { t.Fatal("untracked hash") }; if s7u, _ := build(wt, "", wt, "", "", "", evp, ""); s7u.EvidenceBound || !strings.Contains(strings.Join(s7u.Unmapped, ","), "unexpected.txt") { t.Fatal("untracked still bound", s7u.Unmapped, s7u.MaintReason) }; exec.Command("git", "-C", wt, "add", "-f", "unexpected.txt").Run(); if s7, _ := build(wt, "", wt, "", "", "", evp, ""); s7.EvidenceBound { t.Fatal("unmapped still bound") }; exec.Command("git", "-C", wt, "rm", "-f", "--cached", "unexpected.txt").Run(); os.Remove(filepath.Join(wt, "unexpected.txt"))
	...
	for _, pth := range b2 { b := filepath.Base(pth); if b != allow && b != filepath.Base(decoy) { t.Fatal("dirty candidate compiled", a, b2, allow) } }; if st, err := os.Stat(decoy); err != nil || st.Size() != 1 { t.Fatal("decoy consumed dirty", decoy) }; os.Remove(decoy); if s2, _ := build(wt, "", wt, "", "", "", evp, ""); s2.EvidenceBound { t.Fatal("dirty kernel still bound") }
	pref := t.TempDir(); mini(t, pref, map[string]string{"alpha.md": featBody("alpha", " `.ai/shared/memory/{architecture,conventions}.md`", "::exists scripts/verify-agent-setup", "e")}); os.MkdirAll(filepath.Join(pref, "scripts"), 0755); os.WriteFile(filepath.Join(pref, "scripts/verify-agent-setup"), []byte("x"), 0755); os.WriteFile(filepath.Join(pref, "scripts/verify-agent"), []byte("x"), 0755); os.MkdirAll(filepath.Join(pref, ".ai/agents"), 0755); os.WriteFile(filepath.Join(pref, ".ai/agents/ghost.md"), []byte("x"), 0644); os.MkdirAll(filepath.Join(pref, ".ai/shared/memory"), 0755); os.WriteFile(filepath.Join(pref, ".ai/shared/memory/architecture.md"), []byte("x"), 0644); os.WriteFile(filepath.Join(pref, ".ai/shared/memory/conventions.md"), []byte("x"), 0644); os.WriteFile(filepath.Join(pref, ".ai/shared/memory/other.md"), []byte("x"), 0644); exec.Command("git", "init", "-q", pref).Run(); exec.Command("git", "-C", pref, "config", "user.email", "t@t").Run(); exec.Command("git", "-C", pref, "config", "user.name", "t").Run(); exec.Command("git", "-C", pref, "add", "-A").Run(); exec.Command("git", "-C", pref, "commit", "-q", "-m", "t").Run(); su, _ := build(pref, pref, pref, "", "", "", "", ""); u := strings.Join(su.Unmapped, ","); if !strings.Contains(u, "scripts/verify-agent") || strings.Contains(u, "scripts/verify-agent-setup") || !strings.Contains(u, ".ai/agents/ghost.md") || strings.Contains(u, "architecture.md") || strings.Contains(u, "conventions.md") || !strings.Contains(u, "other.md") { t.Fatal("prefix sweep", su.Unmapped) }; h0 := mapHash(pref); os.WriteFile(filepath.Join(pref, ".ai/shared/memory/other.md"), []byte("y"), 0644); if mapHash(pref) != h0 { t.Fatal("brace hash") }
}

...
func TestLinkedWorktreeHook(t *testing.T) {
	...
	pref := t.TempDir(); mini(t, pref, map[string]string{"alpha.md": featBody("alpha", "", "::exists scripts/verify-agent-setup", "e")}); os.MkdirAll(filepath.Join(pref, "scripts"), 0755); os.WriteFile(filepath.Join(pref, "scripts/verify-agent-setup"), []byte("x"), 0755); os.WriteFile(filepath.Join(pref, "scripts/verify-agent"), []byte("x"), 0755); os.MkdirAll(filepath.Join(pref, ".ai/agents"), 0755); os.WriteFile(filepath.Join(pref, ".ai/agents/ghost.md"), []byte("x"), 0644); exec.Command("git", "init", "-q", pref).Run(); exec.Command("git", "-C", pref, "config", "user.email", "t@t").Run(); exec.Command("git", "-C", pref, "config", "user.name", "t").Run(); exec.Command("git", "-C", pref, "add", "-A").Run(); exec.Command("git", "-C", pref, "commit", "-q", "-m", "t").Run(); if su, _ := build(pref, pref, pref, "", "", "", "", ""); !strings.Contains(strings.Join(su.Unmapped, ","), "scripts/verify-agent") || strings.Contains(strings.Join(su.Unmapped, ","), "scripts/verify-agent-setup") || !strings.Contains(strings.Join(su.Unmapped, ","), ".ai/agents/ghost.md") { t.Fatal("prefix sweep", su.Unmapped) }
}

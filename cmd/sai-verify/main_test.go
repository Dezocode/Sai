...
func TestLinkedWorktreeHook(t *testing.T) {
	...
	hook(); a, _ := filepath.Glob(glob); for _, pth := range a { if bs == "" || filepath.Base(pth) != allow { t.Fatal("bootstrap compiled candidate", a, allow) } }; decoy := filepath.Join(gds, "sai-verify-"+hs+"-cafec0decafe0000"); os.WriteFile(decoy, []byte("x"), 0755); d, _ := filepath.Glob(glob); hit := false; for _, pth := range d { if filepath.Base(pth) != allow { hit = true } }; os.Remove(decoy); if !hit { t.Fatal("decoy accepted as BASE", d, allow) }
	...
	for _, pth := range b2 { if bs == "" || filepath.Base(pth) != allow { t.Fatal("dirty candidate compiled", a, b2, allow) } }; if s2, _ := build(wt, "", wt, "", "", "", evp, ""); s2.EvidenceBound { t.Fatal("dirty kernel still bound") }

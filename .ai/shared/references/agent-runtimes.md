...
|---|---|---|---|
| `runtimes/cursor/` | `cursor-desktop`, `cursor-cloud-vm` | `.cursor/rules/sai-coordination.mdc` + `@<name>`; project plugins via `.cursor/settings.json` (index: `.ai/plugins/`); project Custom Mode `/lauren-mode` (alias `/lauren`) via `.cursor/skills/`; discovery rule `.cursor/rules/lauren-mode.mdc`; pstack role models via `.cursor/rules/pstack-models.mdc` | Cursor Automations UI |

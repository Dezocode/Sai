...
|---|---|---|---|
| `runtimes/cursor/` | `cursor-desktop`, `cursor-cloud-vm` | `.cursor/rules/sai-coordination.mdc` + `@<name>`; project plugins via `.cursor/settings.json` (index: `.ai/plugins/`); project Custom Mode `/lauren-mode` via `.cursor/skills/`; pstack role models via `.cursor/rules/pstack-models.mdc` | Cursor Automations UI |

...
Exact-HEAD: `go test -race ./...`; `go vet ./...`; `go run ./cmd/sai-verify doctor|proof`. Unreachable: live gateway (`openclaw` CLI), ingest/MCQ/run-all stub exit 2.
Maintain pass: source wave + live drive of mapped recipes. Saul P1s remain Hostinger toolchain (no gcc / `go -version`); do not wait. Next: rebase #68 after merge; re-sweep IDs after any new concrete surface.

...
- **Proof.** ::sai proof
## Gotchas
- Recompute from repo+HEAD; conversation memory is not proof. Success hooks omit `permission` so they cannot override an adjacent deny. Matcher `.*` required; empty matcher fails hooks_ok. Cloud Agents do not run sessionStart/End, before/afterMCPExecution, or workspaceOpen; Desktop/CLI do. Protected delete/rewrite fails preserve once BASE has the kernel. HEAD `Removal-authorized:` is ignored. Evidence `map_hash` covers watched implementation surfaces plus `.ai/agents`, `.ai/projects`, `.ai/contracts`, `.ai/shared/memory`, and `openclaw-dashboard`, so a dirty verifier, registry, or proof target cannot keep a prior receipt bound. Completeness sweep matches git paths exactly or by an explicit glob/brace prefix, never by substring. `scripts/agent-report` recipes require `read=`/`has=` and a new queued file after exec. Stale-hook recovery is exactly `go run ./cmd/sai-verify drive` or `proof` with no extra flags; drive writes `--evidence` only under HEAD, gitdir, `os.TempDir()`, `/tmp/`, or `RUNNER_TEMP`. The Go kernel denies mutations when evidence is unbound. Receipts fail closed unless `repo`, BASE, HEAD, and `map_hash` are present and match. Flags that take a value error when the value is missing. Local hook runs a BASE-built kernel when BASE has `cmd/sai-verify` and does not compile candidate Go during bootstrap (bootstrap injects context and does not deny; otherwise the introducing PR cannot land). Policy rejects `func init(` and `plugin.Open` in non-test kernel sources.

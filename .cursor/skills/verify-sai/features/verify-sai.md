...
- `vs-hook-pre` `.cursor/hooks.json` `preToolUse` matcher `.*` `failClosed: true` → `.cursor/hooks/sai-verify.sh` (deny on map/preserve/stale HEAD; FEATURE CONTEXT; no `permission: allow`).

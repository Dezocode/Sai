...
- `ci-feature-maps-pages` `.github/workflows/feature-maps-pages.yml` builds Origin-light HTML from HEAD `.cursor/skills/verify-sai/features/` via `scripts/render-sai-feature-maps`; `main` deploy is non-required GitHub Pages. Also emits `pr-sessions.html`: scrollable PR wheel of live Hermes sessions fetched client-side from the public sessions API, grouped one card per PR with a summary/graph row above the wheel (counts by status plus heartbeat freshness) and stale-heartbeat styling (30m threshold); renderer `--check` asserts tab link, wheel, summary mount, 60s client refresh, and banned-token absence; renderer stays token-free.
...
- `pr-sessions.html` is a client-side consumer of the public sessions API. The renderer never holds a sessions write token; missing session data must render as unavailable, never fabricated. Stale-heartbeat styling is a rendering judgment (30m threshold), not invented data.

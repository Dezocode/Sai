...
decide() {  # policy: prototype tier refuses nothing silently; each rule names itself
  case "$decision" in
    wake-proof|inbox-drain|flightboard-attrib|debugger-launch|channel-probe|owner-steer) printf 'allow' ;;  # owner-steer: landing-page prompt/steer injection (spec §0.5)
